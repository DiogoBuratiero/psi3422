...
static inline int key_at_xy_on_keyboard(int x, int y) {
  ...
  if (0 <= l && l < 3 && 0 <= c && c < 3) return kmap[l][c];
  return 0;
}

static void on_mouse(int event, int x, int y, int /*flags*/, void *) {
  const bool sobreTeclado = (x >= 0 && x < g_cols && y >= 0 && y < g_rows);
  if (event == cv::EVENT_LBUTTONDOWN) { g_mouseDown = true; g_pressed = sobreTeclado ? key_at_xy_on_keyboard(x, y) : 0; }
  else if (event == cv::EVENT_MOUSEMOVE) { if (g_mouseDown) g_pressed = sobreTeclado ? key_at_xy_on_keyboard(x, y) : 0; }
  else if (event == cv::EVENT_LBUTTONUP) { g_mouseDown = false; g_pressed = 0; }
}
...
static cv::Mat makeKeyboard(int w, int h, int activeKey) {
  ...
  for(int r=0;r<3;++r) for(int c=0;c<3;++c) rectangle(kb, Rect(c*cellW,r*cellH,cellW,cellH), gridCol, gridT);

  auto cellRC=[&](int r,int c){ return Rect(c*cellW,r*cellH,cellW,cellH); };
  auto center=[&](const Rect& rc){ return cv::Point(rc.x+rc.width/2, rc.y+rc.height/2); };
  auto isActive=[&](int v){ return activeKey==v; };
...
using namespace cv;
struct Cand { int l=0,c=0,k=0; float cc=-1.f, ncc=-1.f; };

static vector<double> geoScales(double s_min, double s_max, int N){
  vector<double> s(N); double g=std::pow(s_max/s_min,1.0/(N-1)), v=s_min;
  for(int i=0;i<N;++i){ s[i]=v; v*=g; } return s;
}
static void suppressNeighborhood(Mat_<float> &R,int l,int c,int r,float val=0.f){
  ...
  for(int y=L;y<=Rl;++y) for(int x=C;x<=Cr;++x){ int dy=y-l,dx=x-c; if(dy*dy+dx*dx<=r*r) R(y,x)=val; }
}
static vector<Cand> topKWithSeparation(const vector<Mat_<float>>&ccMaps,const vector<Size>&tsz,int K,int minDist){
  vector<Cand> out; vector<Mat_<float>> maps; maps.reserve(ccMaps.size());
  for(auto &m:ccMaps) maps.push_back(m.clone());
  for(int t=0;t<K;++t){
    float best=-1.f; int bk=-1,bl=-1,bc=-1;
    for(int k=0;k<(int)maps.size();++k){ double minv,maxv; Point minp,maxp; minMaxLoc(maps[k],&minv,&maxv,&minp,&maxp);
      if(maxv>best){ best=(float)maxv; bk=k; bl=maxp.y; bc=maxp.x; } }
    if(bk<0) break;
    out.push_back({bl,bc,bk,best,-1.f});
    for(int k=0;k<(int)maps.size();++k) suppressNeighborhood(maps[k],bl,bc,minDist,-1.f);
  }
  return out;
}
...
int main(int argc, char *argv[]) {
  if (argc < 2 || argc > 4) { std::cerr << "uso: cliente4 servidorIp [videosaida.avi] [t/c]\n"; return 1; }
  ...
  { BYTE start='0'; c.sendBytes(1,&start); }

  // ---------- Carrega modelo ----------
  Mat_<COR> tempColor = imread("include/quadrado.png", 1); // ajuste o caminho se necessário
  if (tempColor.total()==0) erro("Erro: nao encontrou quadrado.png");
  Mat_<FLT> Tfloat; converte(tempColor, Tfloat);

  const int NS=10;
  double s_max=69.0/Tfloat.cols, s_min=19.0/Tfloat.cols;
  vector<double> S=geoScales(s_min,s_max,NS);
  vector<Mat_<FLT>> Tcc(NS), Tncc(NS); vector<Size> Tsize(NS);

#pragma omp parallel for schedule(static)
  for (int i=0;i<NS;++i){
    Mat_<FLT> Tr; resize(Tfloat, Tr, Size(), S[i], S[i], INTER_NEAREST);
    Tsize[i]=Tr.size(); Tcc[i]=somaAbsDois(dcReject(Tr,1.0f)); Tncc[i]=Tr.clone();
  }

  cv::VideoWriter wr; bool wrOpen=false;
  int fourcc=cv::VideoWriter::fourcc('M','J','P','G'); double fpsHint=18.0;

  // ---------- Parâmetros de decisão ----------
  const float THRESH_NCC = 0.55f; // confiança
  const float CENTER_BAND = 0.20f; // *** largura relativa da faixa central (±15%) ***

  BYTE out='0'; int frames=0; double t1=nowSec();

  while (true) {
    Mat_<COR> cam; c.receiveImgComp(cam);
    g_cols=cam.cols; g_rows=cam.rows;

    // ---------- Localização ----------
    Mat_<FLT> f; converte(cam, f);
    vector<Mat_<float>> Rcc(NS), Rncc(NS);

#pragma omp parallel for schedule(dynamic)
    for (int i=0;i<NS;++i) Rcc[i]=matchTemplateSame(f, Tcc[i], TM_CCORR, 0.0f);

    vector<Cand> cands=topKWithSeparation(Rcc, Tsize, /*K=*/20, /*minDist=*/10);

#pragma omp parallel for schedule(dynamic)
    for (int i=0;i<NS;++i) Rncc[i]=matchTemplateSame(f, Tncc[i], TM_CCOEFF_NORMED, 0.0f);

    Cand best; bool found=false;
    for (auto &p:cands){
      float v=Rncc[p.k](p.l,p.c); p.ncc=v;
      if(!found || v>best.ncc){ best=p; found=true; }
    }

    // ---------- Decide comando (apenas leve ESQ/DIR; centro = frente) ----------
    char cmd_auto='0';
    Mat telaCam=cam.clone();
    // *** desenhar limiares da faixa central ***
    const float cx = cam.cols * 0.5f;
    const int xL = int(cx - CENTER_BAND * cam.cols);
    const int xR = int(cx + CENTER_BAND * cam.cols);
    cv::line(telaCam, {xL,0}, {xL,cam.rows-1}, cv::Scalar(0,255,0), 1, cv::LINE_AA); // limiar esquerdo
    cv::line(telaCam, {xR,0}, {xR,cam.rows-1}, cv::Scalar(0,255,0), 1, cv::LINE_AA); // limiar direito

    if (found && best.ncc >= THRESH_NCC) {
      float ex = (best.c - cx) / float(cam.cols); // erro relativo [-0.5,0.5]
      if (std::fabs(ex) <= CENTER_BAND)      cmd_auto = '8'; // *** centro -> frente ***
      else if (ex < 0)                       cmd_auto = '7'; // *** esquerda leve ***
      else                                   cmd_auto = '9'; // *** direita leve ***
      ...
      // *** HUD simples ***
      char hud[128];
      std::snprintf(hud, sizeof(hud), "ex=%+.3f  cmd=%c  band=%.0f%%", ex, cmd_auto, CENTER_BAND*100.0f);
  ...
  if(wrOpen) wr.release();
  double dt=nowSec()-t1; if(dt>0) std::printf("Quadros=%d tempo=%.2fs fps=%.2f\n", frames, dt, frames/dt);
  return 0;
}
