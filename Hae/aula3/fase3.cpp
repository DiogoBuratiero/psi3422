...
try
{
  ...
  VideoCapture vi(vin);
  if (!vi.isOpened())
    erro("Erro: Abertura de video");

  // total de quadros (pode ser 0 em alguns contêineres)
  ...
  while (true)
  {
    ...
    if (found && best.ncc >= THRESH_NCC)
      ...
      drawCandidates(out, cands, Tsize, nullptr);
  ...
  {
    std::fprintf(stderr, "\r");
    printProgressBar(totalFrames, totalFrames);
  }
  std::fprintf(stderr, "\n");
