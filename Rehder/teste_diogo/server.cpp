...
int main()
{
  ...
  // Cria odometria
  Odometry odom(AXLE_WIDTH, MM_PER_PULSE_RIGHT, MM_PER_PULSE_LEFT);
  ...
  while (true)
  {
    ...
    if (t.read_ms() >= TELEMETRY_PERIOD_MS)
    {
      ...
      // Atualiza odometria com os pulsos absolutos
      odom.update(cR, cL);
      ...
      // Debug local
      pc_printf("[DIST] R_p=%ld  L_p=%ld  R=%s cm  L=%s cm  |  Pose: x=%.1fmm y=%.1fmm th=%.2frad\r\n",
                (long)cR, (long)cL, sR, sL, pose.x_mm, pose.y_mm, pose.theta_rad);
